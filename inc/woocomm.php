...
if ( function_exists( 'is_woocommerce' ) ) {
  ...
  //woocommerce wrappers
	...
	function pulse_wrapper_start() {
		echo '<section id="content" class="grid-x site-main woocomm-wrap" role="main">';
		echo '<div class="cell small-12 large-padding">';
	}
	function pulse_wrapper_end() {
		echo '</div>';
		echo '</section>';
	}
	add_action('woocommerce_before_main_content', 'pulse_wrapper_start', 10);
	add_action('woocommerce_after_main_content', 'pulse_wrapper_end', 10);
	...
	function pulse_woocommerce_image_dimensions() {
		global $pagenow;
	...
	add_action( 'after_switch_theme', 'pulse_woocommerce_image_dimensions', 1 );

	//remove the breadcrumb (on product pages)
	function pulse_remove_wc_breadcrumbs() {
			remove_action( 'woocommerce_before_main_content', 'woocommerce_breadcrumb', 20, 0 );
	}
	add_action( 'init', 'pulse_remove_wc_breadcrumbs' );
  ...
  add_action( 'woocommerce_before_cart', 'woocommerce_checkout_login_form' );
	...
	function pulse_place_order_button_text() { return __( 'Complete Registration', 'woocommerce' ); }
	add_filter( 'woocommerce_order_button_text', 'pulse_place_order_button_text' );
  ...
  //change add to cart button text
	function pulse_woocommerce_product_add_to_cart_text(){
		return 'Register';
	}
	add_filter( 'woocommerce_product_add_to_cart_text' , 'pulse_woocommerce_product_add_to_cart_text' );
	add_filter( 'woocommerce_product_single_add_to_cart_text', 'pulse_woocommerce_product_add_to_cart_text' );
	add_filter( 'woocommerce_booking_single_add_to_cart_text', 'pulse_woocommerce_product_add_to_cart_text' );
...
function pulse_wc_scripts_styles() {
	if( is_admin() ) return;
...
add_action( 'wp_enqueue_scripts', 'pulse_wc_scripts_styles', 100);
...
function g365_add_event_id_reference() {
	wp_enqueue_style( 'foundation-admin', site_url( '/wp-content/themes/pulse-press/css/', 'https' ) . "style-admin.css?ver=2.4" );
...
function pulse_cart_process(){
  ...
  if( !empty(WC()->cart->get_cart()) ) {
    ...
    foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
      ...
      $extra_types = array( 'fee', 'gear', 'clinics', 'tryouts', 'beach', 'beach_tryouts', 'yvl', 'club', 'parentInfo', 'boysclub' );
             ...
             case 'tryouts':
              ...
              for ($i = 1; $i <= $cart_item['quantity']; ++$i) {
                ...
                $extra_fields[] = array(
                  ...
                  'title' => $product_name,
                  'type'  => 'select',
                  'placeholder'=> 'Preferred practice date',
                  'label' => 'Preferred practice date' ,
                  'name'  => 'preferred_practice_date_' ,
                  'required'=> true,
                  'class' => array($product_cat_slug, 'medium-margin-bottom'),
                  'options' => array(
              ...
              for ($i = 1; $i <= $cart_item['quantity']; ++$i) {
                ...
                $extra_fields[] = array(
                  ...
                  'title' => $product_name,
                  'type'  => 'select',
                  'placeholder'=> 'Player Shirt Size',
                  'label' => 'Player Shirt Size' ,
                  'name'  => 'player_shirt_size_' ,
                  'required'=> true,
                  'class' => array($product_cat_slug, 'medium-margin-bottom'),
                  'options' => array(
                    'y_sm' => 'Youth Small',
...
//                 $extra_fields[] = array('slug' => 'club');
                ...
                $extra_fields[] = array(
                  ...
                  'title' => $product_name,
                  'type'  => 'text',
                  'placeholder'=> 'Primary Player Position',
                  'label' => 'Primary Player Position ' ,
                  'name'  => 'primary_player_position_' ,
                  'required'=> true,
                  'class' => array($product_cat_slug)
                );
                $extra_fields[] = array(
                  ...
                  'title' => $product_name,
                  'type'  => 'text',
                  'placeholder'=> 'Secondary Player Position',
                  'label' => 'Secondary Player Position ' ,
                  'name'  => 'secondary_player_position_' ,
                  'required'=> true,
                  'class' => array($product_cat_slug)
                );
                $extra_fields[] = array(
                  ...
                  'title' => $product_name,
                  'type'  => 'text',
                  'placeholder'=> 'Uniform Top Size',
                  'label' => 'Uniform Top Size ' ,
                  'name'  => 'uniform_top_size_' ,
                  'required'=> true,
                  'class' => array($product_cat_slug)
                );
                $extra_fields[] = array(
                  ...
                  'title' => $product_name,
                  'type'  => 'text',
                  'placeholder'=> 'Uniform Bottom Size',
                  'label' => 'Uniform Bottom Size ' ,
                  'name'  => 'uniform_bottom_size_' ,
                  'required'=> true,
                  'class' => array($product_cat_slug)
                );
                $extra_fields[] = array(
                  ...
                  'title' => $product_name,
                  'type'  => 'text',
                  'placeholder'=> 'Desired Uniform Number',
                  'label' => 'Desired Uniform Number ' ,
                  'name'  => 'desired_uniform_number_' ,
                  'required'=> true,
                  'class' => array($product_cat_slug)
                );
//                 $extra_fields[] = array('slug' => 'parentInfo');
                $extra_fields[] = array(
                  'type'  => $product_cat_slug,
                  'title' => $product_name,
                  'type'  => 'text',
                ...
                $extra_fields[] = array(
                  ...
                  'title' => $product_name,
                  'type'  => 'text',
                  'placeholder'=> 'Primary Parent Email',
                  'label' => 'Primary Parent Email ' ,
                  'name'  => 'primary_parent_email_' ,
                  'required'=> true,
                  'class' => array($product_cat_slug)
                );
                $extra_fields[] = array(
                  ...
                  'title' => $product_name,
                  'type'  => 'text',
                  'placeholder'=> 'Primary Parent Number',
                  'label' => 'Primary Parent Number ' ,
                  'name'  => 'primary_parent_number_' ,
                  'required'=> true,
                  'class' => array($product_cat_slug)
                );
                $extra_fields[] = array(
                ...
                $extra_fields[] = array(
                  ...
                  'title' => $product_name,
                  'type'  => 'text',
                  'placeholder'=> 'Secondary Parent Email',
                  'label' => 'Secondary Parent Email ' ,
                  'name'  => 'secondary_parent_email_' ,
                  'required'=> true,
                  'class' => array($product_cat_slug)
                );
                $extra_fields[] = array(
                  ...
                  'title' => $product_name,
                  'type'  => 'text',
                  'placeholder'=> 'Secondary Parent Number',
                  'label' => 'Secondary Parent Number ' ,
                  'name'  => 'secondary_parent_number_' ,
                  'required'=> true,
                  'class' => array($product_cat_slug)
                );
              }
...
//                 $extra_fields[] = array('slug' => 'club');
                ...
                $extra_fields[] = array(
                  ...
                  'type'  => 'text',
                  'placeholder'=> 'Uniform Bottom Size ',
                  'label' => 'Uniform Bottom Size (slunks) ' ,
                  'name'  => 'uniform_bottom_size_' ,
                  'required'=> true,
                  'class' => array($product_cat_slug)
                );
...
add_filter( 'woocommerce_before_cart' , 'pulse_cart_setup');
function pulse_cart_setup() {
  //load in global proc var
  global $cart_parse;
  //process cart items
  $cart_parse = pulse_cart_process();
  ...
  if(!empty($cart_parse[2]) ){
    ...
    add_filter( 'woocommerce_after_order_notes' , function( $fields ) use ( $order_data_field ) { return pulse_player_checkout_field( $field, $order_data_field ); });
  }
}
...
function g365_order_data_form_field($no_parameter, $key, $args, $value) {
  ...
  foreach( $args['g365_cart_types'] as $dex => $type ){
    switch( $type ){
      ...
      case 'club_team':
        $current_user = wp_get_current_user();
        $presets[] = $type . '_preset,user_ac:' . ((strpos(site_url(), 'dev') === false) ? 'pulse' : 'OGD') . '-' . $current_user->ID;
        break;
    }
  }
...
function pulse_player_checkout_field( $checkout, $extra_fields = null ) {
...
//     echo $dex;
  ...
  echo '<input type="hidden" name="pulse_extra_data" value="' . implode(',',$added_field_names) . '" />';
}

//make the extra data required based on the field names variable
add_action('woocommerce_checkout_process', 'pulse_player_checkout_field_process');
function pulse_player_checkout_field_process() {
  // Check if we have extra data
  if ( $_POST['pulse_extra_data'] ) {
    //parse and loop through the set to make sure we have all the fields filled out
    $fields = explode(',', $_POST['pulse_extra_data']);
...
add_action( 'woocommerce_checkout_update_order_meta', 'pulse_player_checkout_field_update_order_meta' );
function pulse_player_checkout_field_update_order_meta( $order_id ) {
  // Check if we have extra data
  if ( $_POST['pulse_extra_data'] ) {
    //save the field reference so we know what to pull later
    update_post_meta( $order_id, 'pulse_extra_data', sanitize_text_field( $_POST['pulse_extra_data'] ) );
...
add_action( 'woocommerce_admin_order_data_after_billing_address', 'pulse_player_checkout_field_display_admin_order_meta', 10, 1 );
function pulse_player_checkout_field_display_admin_order_meta($order){
  //parse the field names, if we have them, then echo them
  $fields = get_post_meta( $order->id, 'pulse_extra_data', true );
  if( !empty($fields) ) {
...
function pulse_edit_memberships_actions( $actions ) {
...
add_filter( 'wc_memberships_members_area_my-memberships_actions', 'pulse_edit_memberships_actions' );
add_filter( 'wc_memberships_members_area_my-membership-details_actions', 'pulse_edit_memberships_actions' );
    ...
    foreach ( $product_ids as $product_id ) {
      ...
      if( !empty($series_events[ $product_id ][ 'data_pull' ]) ) {
        ...
        if( empty($series_events[ $product_id ][ 'data_pull' ]->dates) ) {
          ...
          //add to the series order so we can be organized
          $series_order[ $product_id ] = date( 'Y-m-d', strtotime(pulse_build_dates($series_events[ $product_id ][ 'data_pull' ]->dates, 4)));
          ...
          if( $payment_options ) {
            ...
            $series_events[ $product_id ][ 'label' ][] = pulse_build_dates($series_events[ $product_id ][ 'data_pull' ]->dates, 1, true);
...
add_filter( 'woocommerce_checkout_fields' , 'pulse_modify_checkout_fields' );
// Our hooked in function - $fields is passed via the filter!
function pulse_modify_checkout_fields( $fields ) {
   unset($fields['order']['order_comments']);
   return $fields;
}

//validate the checkout form before finishing the process of writting data to pulse about the purchase
add_action('woocommerce_after_checkout_validation', 'process_pulse_data');
function process_pulse_data($form_data) {
...
add_action( 'woocommerce_checkout_update_order_meta', 'pulse_update_meta_form_fields' );
function pulse_update_meta_form_fields( $order_id ) {
...
add_action( 'show_user_profile', 'pulse_user_fields_display' );
add_action( 'edit_user_profile', 'pulse_user_fields_display' );

function pulse_user_fields_display( $user ) {
	wp_enqueue_style( 'foundation-admin', site_url( '/wp-content/themes/pulse-press/css/', 'https' ) . "style-admin.css?ver=2.345" );
	wp_enqueue_script( 'admin-jquery', 'https://ajax.googleapis.com/ajax/libs/jquery/2.2.4/jquery.min.js', array(), '2.2.4', true );
  wp_enqueue_script( 'js-g365-all', (( strpos(site_url(), 'dev.') === false ) ? 'https://grassroots365.com/data-processor.js' : 'https://dev.grassroots365.com/data-processor.js'), array('admin-jquery'), '69550', true );
  ?>
	<h3><?php esc_html_e( 'Additional User Data', 'pulse' ); ?></h3>
  <div class="options_group g365_manage_wrapper">
...
add_action( 'user_profile_update_errors', 'pulse_user_field_error', 10, 3 );
function pulse_user_field_error( $errors, $update, $user ) {
	if ( ! $update ) {
		return;
	}
}

add_action( 'personal_options_update', 'pulse_user_fields_update' );
add_action( 'edit_user_profile_update', 'pulse_user_fields_update' );

function pulse_user_fields_update( $user_id ) {
