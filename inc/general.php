...
function pl_waiver_columns( $columns ) {
  ...
  $columns['pulse_user_name'] = 'User Name';
  $columns['pulse_user_email'] = 'User Email';
  return $columns;
}
add_filter( 'manage_nf_sub_posts_columns', 'pl_waiver_columns' );

function pl_waiver_column( $column, $sub_id ) {
  if( $column === 'pulse_user_name' || $column === 'pulse_user_email' ) {
    //get submission data
    $sub = Ninja_Forms()->form()->get_sub( $sub_id );
    //pull user info
    $user = $sub->get_user();
    //add it in to the column
    if( $column === 'pulse_user_name') echo $user->data->user_nicename;
    if( $column === 'pulse_user_email') echo $user->data->user_email;
  }
}
...
function add_apple_touch_icons(){
	...
	echo '<link rel="icon" type="image/png" href="' . get_template_directory_uri() . '/assets/tiny-logos/logo-pulse-no-circle-512x512.png" />' . "\n";
...
if ( ! function_exists( 'theme_setup' ) ) :
    ...
    add_theme_support( 'align-wide' );
		...
		load_theme_textdomain( 'pulse-press' );
...
if ( ! function_exists( 'pulse_main_nav' ) ) {
	function pulse_main_nav() {
    ...
    wp_nav_menu( array(
			...
			'fallback_cb'    => false,
			'walker'         => new pulse_Top_Bar_Walker()
		));
	}
}
// Main Navigation Mega
if ( ! function_exists( 'pulse_mega_nav' ) ) {
	function pulse_mega_nav() {
		wp_nav_menu( array(
			'theme_location' => 'main_nav',
			'container'      => false,
			...
			'fallback_cb'    => false,
			'walker'         => new pulse_Mega_Walker()
		));
	}
}
// Main Navigation side Slide
if ( ! function_exists( 'pulse_side_slide_nav' ) ) {
	function pulse_side_slide_nav() {
		wp_nav_menu( array(
			'theme_location' => 'main_nav',
			'container'      => false,
			...
			'fallback_cb'    => false,
			'walker'         => new pulse_Side_Slide_Walker()
		));
	}
}
// Title Nav
if ( ! function_exists( 'pulse_title_nav' ) ) {
	function pulse_title_nav() {
		wp_nav_menu( array(
			'theme_location' => 'title_nav',
			'container'      => false,
...
if ( ! function_exists( 'pulse_single_main_nav' ) ) {
  function pulse_single_main_nav() {
    wp_nav_menu( array(
      'theme_location' => 'title_nav',
      'container'      => false,
...
// Footer Nav
if ( ! function_exists( 'pulse_footer_nav' ) ) {
	function pulse_footer_nav() {
		wp_nav_menu( array(
...
if ( ! function_exists( 'pulse_excerpt' ) ) :
	/**
	 * Displays the optional excerpt.
	 * Wraps the excerpt in a div element.
	 * Create your own pulse_excerpt() function to override in a child theme.
	 * @param string $class Optional. Class string of the div element. Defaults to 'entry-summary'.
	 */
	function pulse_excerpt( $class = 'entry-summary' ) {
...
if ( ! function_exists( 'pulse_excerpt_more' ) && ! is_admin() ) :
/**
 * Replaces "[...]" (appended to automatically generated excerpts) with ... and a 'Continue reading' link.
 * Create your own pulse_excerpt_more() function to override in a child theme.
 * @return string 'Continue reading' link prepended with an ellipsis.
 */
function pulse_excerpt_more() {
	$link = sprintf( '<a href="%1$s" class="more-link">%2$s</a>',
		esc_url( get_permalink( get_the_ID() ) ),
		/* translators: %s: Name of current post */
		sprintf( __( 'Continue reading<span class="screen-reader-text"> "%s"</span>', 'pulse-press' ), get_the_title( get_the_ID() ) )
	);
	return ' &hellip; ' . $link;
}
add_filter( 'excerpt_more', 'pulse_excerpt_more' );
endif;

if ( ! function_exists( 'pulse_entry_meta' ) ) :
/**
 * Prints HTML with meta information for the categories, tags.
 * Create your own pulse_entry_meta() function to override in a child theme.
 */
function pulse_entry_meta() {
	if ( 'posts' === get_post_type() || 'careers_template' === get_post_type() ) {
		$author_avatar_size = apply_filters( 'pulse_author_avatar_size', 49 );
		printf( '<span class="byline"><span class="author vcard">%1$s <span class="screen-reader-text">%2$s </span> <a class="url fn n" href="%3$s">%4$s</a></span></span>',
			get_avatar( get_the_author_meta( 'user_email' ), $author_avatar_size ),
			_x( 'Author', 'Used before post author name.', 'pulse-press' ),
	...
	if ( current_theme_supports( 'post-formats', $format ) ) {
		printf( '<span class="entry-format">%1$s<a href="%2$s">%3$s</a></span>',
			sprintf( '<span class="screen-reader-text">%s </span>', _x( 'Format', 'Used before post format.', 'pulse-press' ) ),
...
// 		pulse_entry_taxonomies();
	}

	if ( ! is_singular() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
		echo '<span class="comments-link">';
		comments_popup_link( sprintf( __( 'Leave a comment<span class="screen-reader-text"> on %s</span>', 'pulse-press' ), get_the_title() ) );
		echo '</span>';
	}
}
endif;

if ( ! function_exists( 'pulse_entry_date' ) ) :
/**
 * Prints HTML with date information for current post.
 * Create your own pulse_entry_date() function to override in a child theme.
 */
function pulse_entry_date() {
...
// 		$time_string = '<time class="entry-date published" datetime="%1$s">%2$s</time> | <time class="updated" datetime="%3$s">%4$s</time>';
	...
	printf( '<span class="posted-on"><span class="screen-reader-text">%1$s </span><a href="%2$s" rel="bookmark">%3$s</a></span>',
		_x( 'Posted on', 'Used before publish date.', 'pulse-press' ),
...
function pulse_admin_menu() {
	add_menu_page( 'pulse Data Admin', 'Data', 'manage_options', 'pulse_data', 'pulse_admin', 'dashicons-chart-line', 26  );
// 	add_submenu_page( 'pulse_data', 'pulse Data Dashboard', 'Dashboard', 'manage_options', 'pulse_data', 'pulse_admin' );
}
add_action( 'admin_menu', 'pulse_admin_menu' );

//build admin page
function pulse_admin(){
	wp_enqueue_style( 'pulse_admin_styles', get_stylesheet_directory_uri() . '/css/pulse-admin-style.css' );
	$g_action = filter_input( INPUT_GET, 'g_action', FILTER_SANITIZE_URL );
	$g_action = ( empty($g_action) ) ? 'settings' : $g_action;
	$pulse_sections = array(
		'settings' => array(
			'title'		=> 'Manage Site Settings',
			'tab_name'	=> 'Settings',
			'url'			=> '?page=pulse_data&g_action=settings'
		)
	);
	echo '<div class="pulse_data_manager_wrapper">';
	$pulse_admin_url = '';
	$nav = '';
	foreach( $pulse_sections as $section => $section_data ) {
		if( $section == $g_action ) {
			echo '<h1>' . $section_data['title'] . '</h1>';
			$pulse_admin_url = $section_data['url'];
	...
	echo '<nav class="nav-tab-wrapper">' . $nav . '</nav><div class="pulse_data_manager_content_wrapper">';
	switch( $g_action ) {
		case 'export_data':
			pulse_export();
			break;
		...
		case 'settings':
			...
			function pulse_update_admin_site_settings( $old, $new ) {
				foreach( $new as $item => $item_data ) {
					foreach( $item_data as $item_name => $item_value ) {
						$old[$item]['items'][$item_name]['value'] = pulse_process_data_point( $old[$item]['items'][$item_name]['type'], $item_value);
            if( $old[$item]['items'][$item_name]['value'] === null || $old[$item]['items'][$item_name]['value'] === 'null' ) $old[$item]['items'][$item_name]['value'] = '';
					}
				}
				return $old;
			}
			foreach( $pulse_admin_settings as $setting_set => &$setting_data ) {
				if( $_POST['g_process'] == 'process' ) {
					$pulse_db_option = $_POST['pulse_admin_form_data'][$setting_set];
					$update = update_option( 'pulse_' . $setting_set, $pulse_db_option );
					$setting_data['section_records'] = pulse_update_admin_site_settings( $setting_data['section_records'], $pulse_db_option );
					if( $update === true ) echo '<p class="success">Updated settings data!</p>';
				} else {
					$pulse_db_option = get_option( 'pulse_' . $setting_set );
					if( $pulse_db_option !== false && !empty($pulse_db_option) ) $setting_data['section_records'] = pulse_update_admin_site_settings( $setting_data['section_records'], $pulse_db_option );
        }
			} ?>
			<h3>General Site Settings</h3>
			<form id="pulse_form" method="post" class="pulse_form" action="<?php echo $pulse_admin_url; ?>">
				<input type="hidden" name="g_process" value="process" />
			<?php foreach( $pulse_admin_settings as $setting_set_re => $setting_data_re ) : ?>
				...
				<table class="pulse_form_section form-table">
					<tbody>
					<?php foreach( $record_data['items'] as $element => $element_data ) {
						$element_data['tag'] = 'pulse_admin_form_data[' . $setting_set_re . '][' . $record . '][' . $element . ']';
						$element_data['description'] = ( empty($element_data['description']) ) ? '' : '<small>' . $element_data['description'] . '</small>';
						echo pulse_template_construction( $element_data );
					} ?>
					</tbody>
				</table>
				<?php endforeach;
...
function g365_data_receiver(){
  $grassroots_keys = get_option( 'pulse_g365_connector' );
...
function send_html_email($email, $subject, $message) {
  $headers = ( strpos( site_url(), 'dev' ) !== false ) ? array('From: pulse Dev Customer Service <no-reply@dev.opengympremier.com>') : array('From: pulse Customer Service <no-reply@opengympremier.com>');
...
function pulse_start_ads( $pageID ){
// 	$pageID = ( $pageID === null ) ? $post->ID : $pageID;
  ...
  $pulse_page_ad = array(
	...
	$pulse_site_ads = get_option( 'pulse_display' );
	//if the page doesn't have an ad or the site global is empty, don't add to array
	$ad_info['go'] = ( empty($pulse_page_ad['link']) || empty($pulse_page_ad['img']) || empty($pulse_site_ads) ) ? false : true;
	...
	if ( $ad_info['go'] ) {
		...
		$pulse_site_ads['site_1']['element_type'] = 'rotator';
		$pulse_site_ads['site_2']['element_type'] = 'rotator';
		//build rotator element for page ad
		$ad_info['ad_content'] .= pulse_template_construction( $pulse_page_ad );
		//if we have a global ad, build and add it
		if( !empty($pulse_site_ads['site_1']['img']) ) $ad_info['ad_content'] .= pulse_template_construction( $pulse_site_ads['site_1'] );
		//if we have a secondary global ad, build if and add it along with another primary ad to help the flow
		if( !empty($pulse_site_ads['site_2']['img']) ) {
			$ad_info['ad_content'] .= pulse_template_construction( $pulse_page_ad );
			$ad_info['ad_content'] .= pulse_template_construction( $pulse_site_ads['site_2'] );
		}
	}
	//on the front page, and if we have a global splash setting
	if( is_front_page() && !empty($pulse_site_ads['site_3']) ) {
		//controls for the splash
		$pulse_splash = array(
			'title'				=> ( empty($pulse_site_ads['site_3']['title']) ) ? '' : $pulse_site_ads['site_3']['title'],
			'link'				=> ( empty($pulse_site_ads['site_3']['link']) ) ? '' : $pulse_site_ads['site_3']['link'],
			'img'					=> ( empty($pulse_site_ads['site_3']['img']) ) ? '' : $pulse_site_ads['site_3']['img'],
			'img_mobile'	=> ( empty($pulse_site_ads['site_3']['img_mobile']) ) ? '' : $pulse_site_ads['site_3']['img_mobile'],
			'description'	=> ( empty($pulse_site_ads['site_3']['description']) ) ? '' : $pulse_site_ads['site_3']['description']
		);
		//if we have minimum info process the splash
		if( !empty($pulse_splash['title']) && !empty($pulse_splash['link']) && !empty($pulse_splash['img']) ) {
			//build the splash html
			$ad_info['splash'] = '<div class="reveal text-center" id="pulse_home_reveal" aria-labelledby="' . $pulse_splash['title'] . '" data-reveal><div class="relative"><h1 id="reveal-title" class="show-for-sr">' . $pulse_splash['title'] . '</h1>';
			//if there is a derscription, add it
			if( !empty($pulse_splash['description']) ) $ad_info['splash'] .= '<p class="reveal-description">'. $pulse_splash['description'] . '</p>';
			//if there is a mobile image, add it
			if( !empty($pulse_splash['img_mobile']) ) $ad_info['splash'] .= '<p class="text-center show-for-small-only"><a href="' . $pulse_splash['link'] . '"><img src="'. $pulse_splash['img_mobile'] . '" alt="'. $pulse_splash['title'] . '" title="'. $pulse_splash['title'] . '" /></a></p>';
			//if there is a mobile image, add supporting class to the large image
			$mobile_img_class = ( empty($pulse_splash['img_mobile']) ) ? '' : ' show-for-medium';
			//add main splash image
			$ad_info['splash'] .= '<p class="text-center' . $mobile_img_class . '"><a href="' . $pulse_splash['link'] . '"><img src="'. $pulse_splash['img'] . '" alt="'. $pulse_splash['title'] . '" title="'. $pulse_splash['title'] . '" /></a></p>';
...
function pulse_template_construction( $ele_data ) {
	$pulse_admin_form_elements = array(
		'input' 	=> array(
			'template' => '<tr>
					<th>
	...
	$ele = $pulse_admin_form_elements[$ele_data['element_type']]['template'];
	foreach( $pulse_admin_form_elements[$ele_data['element_type']]['vars'] as $dex => $var ) {
    if( $var === 'options' ) {
      $all_options = '';
      $option_ele = $pulse_admin_form_elements[$ele_data['element_type']]['template_option'];
...
function pulse_process_data_point( $name, $point ) {
...
function pulse_build_dates($dates, $type = 1, $abbv = false, $add_reg = false) {
    ...
    switch( $type ){
      ...
      case 'date':
        ...
        if( empty($event_object->dates) ) {
          ...
          $date_reg = pulse_build_dates($event_object->dates, 2, false, $reg_days);
          if( $reg_days === false ) {
      ...
      case 'full_address':
        if( empty($event_object->short_locations) ) {
          ...
          $reg_days = $atts['reg_days'];
          $date_reg = pulse_build_dates($event_object->short_locations, 2, false, $reg_days);
          if( $reg_days === false ) {
            $data_compile .= $date_reg;
          } else {
            $data_compile .= $date_reg[0];
